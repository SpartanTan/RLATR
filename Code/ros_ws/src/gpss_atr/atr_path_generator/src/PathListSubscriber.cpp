...
#include <atr_path_generator/PathListSubscriber.h>
...
PathListSubscriber::PathListSubscriber() : Node("atr_path_list_subs")
{
  ...
  atr_path_list_subs_ = create_subscription<atr_path_msgs::msg::ATRPathList>(
...
void PathListSubscriber::init()
{
  ...
  // declare_parameter("topic_name_prefix");

  // declare_parameter<std::string>("atr_path_topic_name", "atr_path_list");
  // declare_parameter<int>("period_ms", 1000);
  // declare_parameter<std::string>("frame_id", "map");
  // // declare_parameter<int>("atr_number", 2);
  // declare_parameter<std::string>("path_marker_topic_name", "atr_paths_marker");

  // atr_path_topic_name_ = get_parameter("atr_path_topic_name").as_string();
  // period_ms_ = get_parameter("period_ms").as_int();
  // frame_id_ = get_parameter("frame_id").as_string();
  // // atr_number_ = get_parameter("atr_number").as_int();
  // marker_topic_name_ = get_parameter("path_marker_topic_name").as_string();
  ...
  declare_parameter<int64_t>(param_names.at(1));
  declare_parameter<std::string>(param_names.at(2));
  declare_parameter<std::string>(param_names.at(3));
  declare_parameter<int64_t>(param_names.at(4));
  declare_parameter<double>(param_names.at(5));
  declare_parameter<double>(param_names.at(6));
