...
#include <vector>
#include "atr_controller/atr_controller.hpp"
#include "apex_test_tools/apex_test_tools.hpp"

// TODO(carlossvg): add out of range tests

using atr::atr_controller::ATRController;

class TestATRController : public ::testing::Test
{
protected:
  std::vector<double> feedback_matrix = { -10.0000, -51.5393, 356.8637, 154.4146 };
  ATRController::Config config{ feedback_matrix };
};

TEST_F(TestATRController, config)
{
...
TEST_F(TestATRController, set_teleoperation)
{
  ATRController controller{ config };
  double cart_pos{ 1.0 };
  double cart_vel{ 2.0 };
  double pole_pos{ 3.0 };
  double pole_vel{ 4.0 };
...
TEST_F(TestATRController, set_state)
{
  ATRController controller{ config };
  double cart_pos{ 1.0 };
  double cart_vel{ 2.0 };
...
TEST_F(TestATRController, set_force_command)
{
  ATRController controller{ config };
...
TEST_F(TestATRController, init_state)
{
  ATRController controller{ config };
...
TEST_F(TestATRController, reset)
{
  ATRController controller{ config };
...
TEST_F(TestATRController, update)
{
  ATRController controller{ config };
  apex_test_tools::memory_test::start();
  controller.update();
