...
def generate_launch_description():
    ...
    # The atr_id_list parameter is a global parameter used by different packages. In this case, the atr bring up
    # node and the atr_factory_sate node share this parameter.
    # We have created a special ros2 package named factory_db to contain all these global parameters
    # We need to load the atr_id_list parameter and pass it to the two nodes
    ...
    atr_factory_state_dir = FindPackageShare("atr_factory_state").find("atr_factory_state")
    atr_factory_state_yaml = os.path.join(atr_factory_state_dir, "config", "atr_factory_state.yaml")
    ...
    # ATR Path List subscriber (visualize Trajectory Generator path)
    atr_path_list_subs_tg = Node(
        package="atr_path_generator",
    ...
    #     emulate_tty=True,
    # )
    # ld.add_action(atr_rl_controller)
    return ld
