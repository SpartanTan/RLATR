...
def create_lifecycle_events(node):
    ...
    return [event_configure, event_activate, event_deactivate, event_clean, event_shutdown]
...
def generate_test_description():
    ...
    controller_node = launch_ros.actions.LifecycleNode(
        package="atr_controller",
        executable="atr_controller_exe",
        name="atr_controller",
        parameters=[param_file],
        arguments=["--autostart", "False"],
        additional_env={"PYTHONUNBUFFERED": "1"},
...
class TestLifecycle(unittest.TestCase):
    ...
    def test_controller_node_transmits(self, controller_node, driver_node, proc_output):
        pub = self.node.create_publisher(atr2_msgs.msg.ATRTeleop, "teleop", 10)
