...
#ifndef JOY2JOINTCMD_H
#define JOY2JOINTCMD_H

/*! \file Joy2JointCMD.h
 *  \brief Transforms joy message to JointCMD message type
 *
 *  Provides the following functionalities:
 *      - Subscriber to Joy message
 *      - Publisher to publish the Joy message transformed into JointCMD message
 */
...
/**
 *  @class This class maps Joy messages to JointCMD messages
 */

class Joy2JointCMD : public rclcpp::Node
{
  // Private variables
private:
  rclcpp::Subscription<sensor_msgs::msg::Joy>::SharedPtr f710Subs_;  ///< Subscriber to get joytick information
  rclcpp::Publisher<atr_state_msgs::msg::ATRJointCommand>::SharedPtr f710TwistPublisher_;  ///< Publisher to publish the
                                                                                           ///< Joystick information
                                                                                           ///< transformed into Twist
                                                                                           ///< message

  Vector3d m_gains_;                                    ///< gain factor to change joystick data [-1,1] to max velocity
  atr_state_msgs::msg::ATRJointCommand m_JointCMDMsg_;  ///< JointCMD message
  std::string m_joyTopicName_;                          ///< Topic name where the joy information will be published
  std::string m_cmdJointName_;  ///< Topic name where the transformed JointCMD information will be published
...
public:
  /**
   * @brief Construct a new Joy 2 JointCMD object
   *
   */
  Joy2JointCMD();

  /**
   * @brief Destroy the Joy 2 JointCMD object
   *
   */
  ~Joy2JointCMD();
