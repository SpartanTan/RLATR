...
#ifndef ATR_UTILS__PROCESS_SETTINGS_HPP_
#define ATR_UTILS__PROCESS_SETTINGS_HPP_
...
#include "atr_utils/memory_lock.hpp"
#include "atr_utils/rt_thread.hpp"
...
struct ProcessSettings
{
  void print_usage()
  {
    RCLCPP_INFO(rclcpp::get_logger("process_settings"),
                "\t[%s auto start nodes]\n"
                "\t[%s lock memory]\n"
                ...
                "\t[-h]\n",
                OPTION_AUTO_ACTIVATE_NODES.c_str(), OPTION_LOCK_MEMORY.c_str(), OPTION_LOCK_MEMORY_SIZE.c_str(),
                OPTION_PRIORITY.c_str(), OPTION_CPU_AFFINITY.c_str(), OPTION_CONFIG_CHILD_THREADS.c_str());
  }
  ...
  bool init(int argc, char* argv[])
  {
    ...
    {
      std::string option = rcutils_cli_get_option(argv, argv + argc, OPTION_AUTO_ACTIVATE_NODES.c_str());

      // RCLCPP_WARN_STREAM(rclcpp::get_logger("process_settings"), "Auto: " << option);

      auto_start_nodes = (option == "True") ? true : false;
    }
    if (rcutils_cli_option_exist(argv, argv + argc, OPTION_LOCK_MEMORY.c_str()))
    {
      std::string option = rcutils_cli_get_option(argv, argv + argc, OPTION_LOCK_MEMORY_SIZE.c_str());
      lock_memory = (option == "True") ? true : false;
    }
    if (rcutils_cli_option_exist(argv, argv + argc, OPTION_LOCK_MEMORY_SIZE.c_str()))
    {
      lock_memory_size_mb = std::stoi(rcutils_cli_get_option(argv, argv + argc, OPTION_LOCK_MEMORY_SIZE.c_str()));
    ...
    {
      process_priority = std::stoi(rcutils_cli_get_option(argv, argv + argc, OPTION_PRIORITY.c_str()));
    }
    if (rcutils_cli_option_exist(argv, argv + argc, OPTION_CPU_AFFINITY.c_str()))
    {
      cpu_affinity = std::stoi(rcutils_cli_get_option(argv, argv + argc, OPTION_CPU_AFFINITY.c_str()));
    }
    if (rcutils_cli_option_exist(argv, argv + argc, OPTION_CONFIG_CHILD_THREADS.c_str()))
    {
      std::string option = rcutils_cli_get_option(argv, argv + argc, OPTION_CONFIG_CHILD_THREADS.c_str());
...
#endif  // ATR_UTILS__PROCESS_SETTINGS_HPP_
